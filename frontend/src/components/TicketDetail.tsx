...
import { ArrowLeft, Clock, CheckCircle, AlertTriangle, User, MapPin, Phone, Home, Building2 } from 'lucide-react';
import { ticketsApi } from '../utils/api';
import Badge from './Badge';
import { formatDate, slaTimeRemaining, isOverdue } from '../utils/helpers';
import type { Ticket, TicketUpdate } from '../types';
import { STATUS_OPTIONS, CATEGORY_COLORS } from '../constants';
...
export default function TicketDetail() {
  ...
  const CatIcon    = ticket.category === 'Personal' ? Home : Building2;
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {/* Category pill */}
              <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ${CATEGORY_COLORS[ticket.category] ?? 'bg-gray-100'}`}>
            ...
            {/* Area breadcrumb for Common tickets */}
            {ticket.category === 'Common' && ticket.area && (
              <p className="text-xs text-gray-400 mt-2 flex items-center gap-1">
                <MapPin className="w-3 h-3" />
                <span className="font-medium text-teal-700">{ticket.area}</span>
                <span className="mx-1">›</span>
                <span className="font-semibold text-gray-700">{ticket.issue_type}</span>
              </p>
            )}
            {ticket.category === 'Personal' && (
              <p className="text-xs text-gray-400 mt-2 flex items-center gap-1">
            ...
            {ticket.sub_category && (
              <p className="text-sm text-gray-500 mt-0.5 ml-4">↳ {ticket.sub_category}</p>
            )}
          </div>
